...
-e MYSQL_ROOT_PASSWORD=P@ssw0rd -v "$(pwd -W):/scripts" -d mysql:8.0-debian
...
docker run --network=test-network -v "$(pwd -W):/repos" --workdir /repos/ -e INSTALL_MYSQL=true \
...
docker run --network=test-network -v "$(pwd -W):/repos" --workdir /repos/ -e INSTALL_MYSQL=true \
    ...
    -e LIQUIBASE_COMMAND_PASSWORD=P@ssw0rd \
    -e LIQUIBASE_COMMAND_URL=jdbc:mysql://mysql:3306/ShopDB \
    liquibase/liquibase liquibase --changelog-file=task.sql tag 0.0.1
...
docker run --network=test-network -v "$(pwd -W):/repos" --workdir /repos/ -e INSTALL_MYSQL=true \
    ...
    -e LIQUIBASE_COMMAND_PASSWORD=P@ssw0rd \
    -e LIQUIBASE_COMMAND_URL=jdbc:mysql://mysql:3306/ShopDB \
    liquibase/liquibase liquibase --changelog-file=task.sql tag 0.0.2
...
docker run --network=test-network -v "$(pwd -W):/repos" --workdir /repos/ -e INSTALL_MYSQL=true \
    ...
    -e LIQUIBASE_COMMAND_PASSWORD=P@ssw0rd \
    -e LIQUIBASE_COMMAND_URL=jdbc:mysql://mysql:3306/ShopDB \
    liquibase/liquibase liquibase --changelog-file=task.sql tag 0.0.3
